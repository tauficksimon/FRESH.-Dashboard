...
def load_orders_data():
    ...
    elif os.path.exists('orders_ALL.csv'):
        # Use the merged file if available
        orders_df = pd.read_csv('orders_ALL.csv')
        return orders_df
    else:
        orders_file = 'orders.csv'
        items_file = 'items.csv'

    # If we need to merge, import the CSVMerger
    if os.path.exists('orders_ALL.csv'):
        orders_df = pd.read_csv('orders_ALL.csv')
    else:
        print("Using pre-merged orders_ALL.csv or monthly CSV files")
        print("If not found, run csv_merge_dashboard.py first")
        sys.exit(1)
